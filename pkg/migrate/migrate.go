...
func (m *Migrate) RunMigrations() error {
	...
	log.Println("HELLO", usableEntries)
	...
	m.txn, err = m.db.BeginTx(context.Background(), pgx.TxOptions{})
	if err != nil {
		return err
	}
	...
	err = m.txn.Commit(context.Background())
	if err != nil {
		return err
	}
...
func (m *Migrate) parseFilesAndMigrateDb() error {
	for _, file := range m.migrationFiles {
		...
		for _, command := range commands {
			_, err = m.txn.Exec(context.Background(),command)
			if err != nil {
				return err
			}
		}
